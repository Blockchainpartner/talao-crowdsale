...
contract('EmindhubCrowdsale', function(accounts) {
  describe('Constructor', () => {
    ...
    before(async () => {
      EmindhubCrowdsaleInstance = await EmindhubCrowdsale.new(startTime,startSale,endTime,goal,presaleCap,cap,wallet, { from: accounts[0], gas:60000000});
    });
  ...
  describe('Sale Features', () => {
    ...
    let teamWallet = "0xE7305033fE4D5994Cd88d69740E9DB59F27c7046";
      ...
      it( 'should give the team their tokens and mint the next round tokens if the sale is successful', async () => {
        ...
        let teamLockedWallet = await EmindhubCrowdsaleInstance.timelockedTokensContracts.call(teamWallet);
        let teamLockedBalance = await expInstance.balanceOf(teamLockedWallet);
        ...
        assert.equal(teamLockedBalance.toNumber(), web3.toWei("15000000"));
        // round should get 30M tokens
        assert.equal(roundLockedBalance.toNumber(), web3.toWei("30000000"));
      });
      ...
      it('should be sucessfully completing the sale', async () => {
        ...
        let lockedTeamWallet = await EmindhubCrowdsaleInstance.timelockedTokensContracts.call(teamWallet);
        let lockedBalanceTeam = await expInstance.balanceOf(lockedTeamWallet);
        assert.equal(balanceTeam.toNumber(), 0);
        assert.equal(lockedBalanceTeam.toNumber(), web3.toWei("15000000"));
      });
